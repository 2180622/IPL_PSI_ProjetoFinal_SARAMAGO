...
<div class="site-cir">
    ...
    <?php
        ...
        elseif(is_numeric($exemplar) && $exemplar == 400)
        {
            echo'<div class="alert alert-danger config" role="alert" id="alert-saramago">
                    <strong>Informação:</strong> O exemplar submetido não está nas condições de ser renovado. Verifique o seu estado.
                </div>';
        }
        elseif($exemplar != null)
        {
            ...
            echo '<div class="menu-nav-saramago">'
                ...
                .Html::button(FAS::icon('sign-out-alt') . ' Fechar sessão', ['onclick'=>"location.href='renovar#go';", 'class' => 'btn btn-alt pull-right'])
                .'</div>';
            #endregion
            #region Table
            ...
            echo '<div class="row container-fluid">
                        <div class="col-md-5">
                            <div class="card card-cir">
                                <div class="card-title"><h4>Leitor...</h4></div>
                                <div class="card-body">
                        ...
                        <div class="col-md-5">
                            <div class="card card-cir">
                                <div class="card-title"><h4>Requisição</h4></div>
                                <div class="card-body">
                                    <p>Nº do Sistema: '.$req->id.'</p>
                                    <p>Data Empréstimo: '.Yii::$app->formatter->asDate($req->dataEmprestimo).'</p>
                                    <p>Data Entrega Prevista: '.Yii::$app->formatter->asDate($req->entregaPrevista).'</p>
                                    <p>Renovações: '.$req->Renovacoes.'</p>
                                </div>
                            </div>
                        </div>
                 </div>';
    ...
    <?php
        $this->registerJs("
                $(function () {
                    if(location.hash == '#go'){
                        $('#rapido-saramago .nav a[href=\"#tab3\"]').tab('show');
                    }
                });
        ");
    ?>
